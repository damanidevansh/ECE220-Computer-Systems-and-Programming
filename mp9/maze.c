  ...
  for ( i=0; i<row; i++){
    for ( j=0; j<col; j++){
      ...
      if (c!='\n')
        ...
        j--;
      if (j>=0){
        if (newmaze->cells[i][j]==START){ //if we are at the start
          newmaze->startRow=i; //assign start row/col the correct number
          newmaze->startColumn=j;
        }
        if (newmaze->cells[i][j]==END){ //if we are at the end
...
// printf("%d %d\n", maze->width, maze->height); //prints size of maze
  for ( i=0; i<maze->height; i++){ //runs through rows
    for ( j=0; j<maze->width;j++){ //runs through cols
